...
export const CoursesAdmin = async (req, res) => {
    try {
        const { title, description, company_name, price,
            isFree, course_link, format, languages, duration, banner_image, brand_image } = req.body;

        if (!title || !description || !company_name || !course_link || !format || !languages
            || !duration || !banner_image || !brand_image) {
            return res.status(400).json({ message: "Please Fill all the fields" })
        } else {
            const newCourse = await Courses.create({
                title, description, company_name, price, isFree, course_link,
...
export const editCourse = async(req,res) => {
    ...
    try {
        const updateCourse = await Courses.findByIdAndUpdate(_id,{$set:{
            ...
            'isFree':isFree,'course_link':course_link,'format':format,'languages':languages,'duration':duration,
